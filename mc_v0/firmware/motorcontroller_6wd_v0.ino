...
Motorcontroller g_mc_front (11, 12, 10, false);
Motorcontroller g_mc_mid   (6,  9,  10, false);
Motorcontroller g_mc_back  (5,  8,  10, false);
...
void loop()
{
  ...
  if(g_serial_speed_rdy)
  {
    ...
    //if(g_cnt_got_speed++ %20 == 0)
    {
      ...
      Serial.print(',');
      Serial.println(g_curr_speed_cmd_left);
...
//      Serial.println(g_millis);
  ...
  g_mc_front.setSpeed(g_curr_speed_cmd_left);
  g_mc_mid  .setSpeed(g_curr_speed_cmd_left);
  g_mc_back .setSpeed(g_curr_speed_cmd_left);

  //check brake
  if(g_serial_brake_rdy)
  {
    int brake_val = ::atoi(g_serial_brake);
    g_mc_front.brk(brake_val);
    g_mc_mid  .brk(brake_val);
    g_mc_back .brk(brake_val);
