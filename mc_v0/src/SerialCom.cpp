...
EnumErrorDevice SerialCom::connect()
{
	...
	try
	{
		...
		//select BoudRate settings 9600...115200 .... may more
...
std::pair<EnumErrorDevice, std::vector<unsigned char> > SerialCom::receive(std::size_t n_bytes)
{
  std::vector<unsigned char> data(n_bytes);

  _port->read_some(buffer(&data.front(), n_bytes));
  return std::make_pair(DeviceSucces, data);
}
...
EnumErrorDevice SerialCom::transmit(const std::vector<unsigned char>& data)
{
  _port->write_some(buffer(&data.front(), data.size()));
  return DeviceSucces;
}

EnumErrorDevice apps::SerialCom::transmit(const std::string& data)
{
  _port->write_some(buffer(data.c_str(), data.size() +1 ));
  return DeviceSucces;
}
...
std::pair<EnumErrorDevice, std::vector<unsigned char> > SerialCom::recieve(std::size_t n_bytes, unsigned int delay_us)
{
  //deadline_timer timer(*_ioService);
  std::vector<unsigned char> myBuffer(n_bytes);
  bool data_available = false;


  _port->async_read_some( boost::asio::buffer(myBuffer, n_bytes),
              boost::bind(&SerialCom::read_callback,
                    this,
                    boost::ref(data_available),
                    boost::ref(*_timer),
  ...
  if (!data_available)
  {
    //no data;
    //std::cout << "debug: receive timed out or failed" << std::endl;
    return std::make_pair( DeviceReceiveFail, std::vector<unsigned char>() );
  }


  return std::make_pair(DeviceSucces, myBuffer);
}
